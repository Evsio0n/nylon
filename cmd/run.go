...
var runCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if cmd.Flags().Changed("advertise-exit-node") {
			opts.AdvertiseExitNode, _ = cmd.Flags().GetBool("advertise-exit-node")
			opts.AdvertiseExitNodeSet = true
		}
		if cmd.Flags().Changed("allow-exit-node") {
			opts.AllowExitNode, _ = cmd.Flags().GetBool("allow-exit-node")
			opts.AllowExitNodeSet = true
		}
		if cmd.Flags().Changed("exit-node") {
			exitNode, _ := cmd.Flags().GetString("exit-node")
			opts.ExitNode = state.NodeId(exitNode)
			opts.ExitNodeSet = true
		}
